...
function showStep(step) {
  const stepData = chatFlow[step]; const messagesDiv = document.getElementById("chatMessages"); const optionsDiv = document.getElementById("chatOptions"); if (stepData.conditional && userResponses.asesoriaSGSST === "No" && step < 3) { currentStep = 3; return showStep(3) }
  messagesDiv.innerHTML += `<div class="message bot-message">${stepData.message}</div>`; optionsDiv.innerHTML = ""; if (stepData.whatsapp) {
    const mensaje = `*Cotización Leal Seguros*\n\n` + `- 🛡️ Asesoría SG-SST: ${userResponses.asesoriaSGSST || "Sin especificar"
...
function handleResponse(response) {
  document.getElementById("chatMessages").innerHTML += `<div class="message user-message">${response}</div>`; switch (currentStep) { case 0: userResponses.asesoriaSGSST = response; break; case 1: userResponses.nivelRiesgo = response; break; case 2: userResponses.trabajadores = response; break; case 3: userResponses.gestionSST = response; break; case 4: userResponses.cambioARL = response; break }
...
function showCopyFeedback(mensajeCodificado, error = null) {
  const feedback = document.createElement("div"); feedback.className = "copy-feedback"; if (error) {
    feedback.innerHTML = `
