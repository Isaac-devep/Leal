...
const chatFlow = [
  {
    message:
      "¡Bienvenido a Leal Asesorías & Seguros! ¿Desea recibir asesoría en sistemas de gestión de seguridad y salud en el trabajo (SG-SST)?",
    options: ["Sí", "No"],
  },
  {
    message: "¿Qué nivel de riesgo psicosocial tiene identificado actualmente?",
  ...
  {
    message: "¿Desea una asesoría en sistemas de gestión SST?",
    options: ["Sí", "No"],
    conditional: !0,
  },
  {
    message: "¿Desea realizar un cambio de ARL?",
    options: ["Sí", "No"],
    conditional: !0,
  },
  {
    message:
      "¡Gracias por sus respuestas! Haga clic para enviar su cotización:",
    whatsapp: !0,
    conditional: !0,
  },
];
function toggleChat() {
  ...
  if (chat.classList.contains("active")) {
    ...
    setTimeout(() => {
      chat.style.display = "none"; // Se oculta completamente después de la animación
      chat.classList.remove("inactive");
    }, 300); // Tiempo suficiente para la animación

    chatIcon.style.display = "flex"; // Mostrar el ícono del chat
  } else {
    chat.style.display = "flex"; // Se muestra antes de animar
    setTimeout(() => chat.classList.add("active"), 10); // Espera 10ms para activar la animación
    chatIcon.style.display = "none"; // Ocultar el icono mientras el chat está abierto
  }
}
function startChat() {
...
function showStep(step) {
  ...
  if (
    stepData.conditional &&
    userResponses.asesoriaSGSST === "No" &&
    step < 3
  ) {
    currentStep = 3;
    return showStep(3);
...
function handleResponse(response) {
  document.getElementById(
    "chatMessages"
  ).innerHTML += `<div class="message user-message">${response}</div>`;
  switch (currentStep) {
    case 0:
      userResponses.asesoriaSGSST = response;
      break;
    case 1:
    ...
    case 2:
      ...
      break;
    case 3:
      userResponses.gestionSST = response;
      break;
    case 4:
...
function copyToClipboard() {
  ...
  navigator.clipboard
    .writeText(mensaje)
    .then(() => {
      showCopyFeedback(mensajeCodificado);
    })
    .catch((err) => {
      showCopyFeedback(null, err);
    });
}
function showCopyFeedback(mensajeCodificado, error = null) {
